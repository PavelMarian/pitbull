"""Offline, checkpointed validation of one A4 repetition/iteration."""
import hashlib
import json
import os
import sys
...
def main():
    ...
    response_dir = RESPONSES / f"repeat_{REPEAT:02d}" / f"iter_{ITERATION:02d}"
    for program in repetition["programs"]:
        ...
        history = {"iteration": ITERATION, "response": str(response_path.relative_to(ROOT)),
                   "fetch_attempts": len(response.get("attempts", []))}
    ...
    counts = {s: sum(p["status"] == s for p in repetition["programs"])
              for s in ("pending", "clean", "failed")}
    print("repeat", REPEAT, counts, flush=True)
